...
### チェインアローのレベルに応じて呼び出し
execute if score _ PotentialSkill matches 0 as @e[distance=..5,tag=!This,tag=!Owner,predicate=tusb_remake:player,sort=random] at @s run function tusb_remake:skill/archer/chain_arrow/hit/damage
execute if score _ PotentialSkill matches 1 as @e[distance=..15,tag=!This,tag=!Owner,predicate=tusb_remake:player,sort=random] at @s run function tusb_remake:skill/archer/chain_arrow/hit/damage
execute if score _ PotentialSkill matches 2..9 as @e[distance=..25,tag=!This,tag=!Owner,predicate=tusb_remake:player,sort=random] at @s run function tusb_remake:skill/archer/chain_arrow/hit/damage
tag @s remove This